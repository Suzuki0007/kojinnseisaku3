...
public:
	...
	// エフェクトを再生(位置指定)
	int PlayEffect3DPos(int handle, const Vec4& pos);
	...
	// 回転
	bool SetRotationEffect(int handle, const Vec4& rad);
	Vec4 GetRotationEffect(int handle) const;

	// 位置設定
	bool SetPosEffect(int handle, const Vec4& pos);
	Vec4 GetPosEffect(int handle) const;

	// 回転設定(度数法)
	bool SetRotationEffectDeg(int handle, const Vec4& deg);
	Vec4 GetRotationEffectDeg(int handle) const;
	...
	// 2Dエフェクト再生(位置指定)
	int PlayEffect2DPos(int handle, const Vec4& pos);
	...
	Vec4 GetScaleEffect2D(int handle) const;

	// 2Dエフェクト位置設定
	bool SetPosEffect2D(int handle, const Vec4& pos);
	Vec4 GetPosEffect2D(int handle) const;

	// 2Dエフェクト回転設定
	bool SetRotationEffect2D(int handle, const Vec4& rad);
	Vec4 GetRotationEffect2D(int handle) const;
...
private:
	...
	std::unordered_map<std::string, int> _effect; // エフェクトパスとハンドルのマップ
	std::unordered_map<int, float> _effectSpeed; // エフェクトハンドルと速度のマップ
	std::unordered_map<int, Vec4> _effectRotation; // エフェクトハンドルと回転のマップ
	std::unordered_map<int, Vec4> _effectPos; // エフェクトハンドルと位置のマップ
